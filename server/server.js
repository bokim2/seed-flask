...
app.get('/api/flasksall/:id', async (req, res) => {
  try {
    const results = await db.query(
      'SELECT * FROM flasks f full outer JOIN samples s ON f.id = s.flask_id LEFT JOIN cell_banks c ON f.cell_bank = c.cell_bank WHERE id = $1 ORDER BY f.id DESC ',
      [req.params.id],
    )
...
app.get('/api/flasks', async (req, res) => {
  try {
    const results = await db.query(
      'SELECT * FROM flasks f full outer JOIN samples s ON f.id = s.flask_id LEFT JOIN cell_banks c ON f.cell_bank = c.cell_bank ORDER BY f.id DESC',
    )
    // LEFT OUTER JOIN cell_bank ON flasks.cell_bank=cell_bank.cell_bank ORDER BY flasks.id
    // console.log(results);
    res.status(200).json({
...
app.get('/api/flasks/:id', async (req, res) => {
  try {
    const results = await db.query('select * from flasks WHERE id = $1', [
      req.params.id,
    ])
...
app.post('/api/flasks', async (req, res) => {
  try {
    ...
    const results = await db.query(
      'INSERT INTO flasks (cell_bank, inoculum_ul, media_ml) values ($1, $2, $3) returning *',
      [req.body.cell_bank, req.body.inoculum_ul, req.body.media_ml],
    )
...
app.post('/api/flasks/:id', async (req, res) => {
  try {
    ...
    let date = new Date()
    const results = await db.query(
      'INSERT INTO samples (end_date, od600, completed, time_since_inoc, flask_id) values ($1, $2, $3, $4, $5) returning *',
      [
        req.body.end_date,
        req.body.od600,
        req.body.completed,
        req.body.time_since_inoc,
        req.body.flask_id,
      ],
    )
...
app.delete('/api/flasks/:id', async (req, res) => {
  try {
    const sample = db.query('DELETE FROM samples WHERE flask_id = $1', [
      req.params.id,
    ])
    const results = db.query('DELETE FROM flasks WHERE id = $1', [
      req.params.id,
    ])
    console.log(results)
    res.status(204).json({
